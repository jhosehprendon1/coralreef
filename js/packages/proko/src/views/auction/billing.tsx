  ...
  useEffect(() => {
    winners.forEach(winner => {
      getBidderPotKey({
        ...
        auctionKey: auctionView.auction.pubkey,
        bidderPubkey: winner.key,
      }).then(key => {
        const el = cache.get(key) as ParsedAccount<BidderPot>;
        if (el) setPots(pots => ({ ...pots, [winner.key.toBase58()]: el }));
      });
    });
  }, [winners]);
  return pots;
          ...
          if (!foundPayoutTickets[key]) {
            getPayoutTicket(
              ...
              if (payoutTickets[payoutKey.toBase58()])
                setFoundPayoutTickets(pt => ({
                  ...pt,
                  [key]: payoutTickets[payoutKey.toBase58()],
                }));
            });
          }
        }
...
export const InnerBillingView = ({
  ...
  console.log('Auction', auctionView);
