...
//const CLUSTER = 'https://api.metaplex.solana.com/';
...
const recursiveReplace = function (json, paths) {
  if (json instanceof Array) {
    return [...json.map(j => recursiveReplace(j, paths))];
  } else if (json instanceof Object) {
    return {
      ...Object.keys(json).reduce((acc, k) => {
        acc[k] = recursiveReplace(json[k], paths);
        return acc;
      }, {}),
    };
  } else if (typeof json == 'string') {
    console.log(json, ' passes string check, with paths', paths);
    if (paths[json]) {
      return `https://www.arweave.net/${paths[json].id}`;
    } else return json;
  } else {
    return json;
  }
...
exports.uploadFile = async (req, res) => {
      ...
      new Promise(async (res, _) => {
        if (fieldname === 'transaction') {
          try {
            ...
            const transaction =
              await solanaConnection.getParsedConfirmedTransaction(
                val,
                'confirmed',
              );
  ...
  busboy.on('finish', async () => {
    ...
    const metadataIndex = filepaths.findIndex(f =>
      f.filepath.match(RESERVED_METADATA),
    );
    if (metadataIndex == -1) {
      body.error = `No ${RESERVED_METADATA} found`;
      res.end(JSON.stringify(body));
      return;
    }

    const metadata = filepaths.splice(metadataIndex, 1)[0];
    // do metadata last once we have all URLs.
    filepaths.push(metadata);
    filepaths.push({ filepath: RESERVED_TXN_MANIFEST, status: SUCCESS });

    const fields = await Promise.all(fieldPromises);
    const anchor = (await arweaveConnection.api.get('tx_anchor')).data;

    console.log('The file name list2 is ' + fields.map(f => f.name).join(','));
      ...
      if (f.status == FAIL) {
        ...
        try {
          ...
          if (runningTotal > 0) {
            ...
            if (filename == RESERVED_METADATA) {
              const asJson = JSON.parse(data);
              console.log('Replacing URLs');
              mutated = recursiveReplace(asJson, paths);
              data = Buffer.from(JSON.stringify(mutated), 'utf8');
            }
