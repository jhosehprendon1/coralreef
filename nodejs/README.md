...
```bash
my-application$ sam local invoke putNftFunction --event events/event-post-nft.json
my-application$ sam local invoke getAllNftsFunction --event events/event-get-all-nfts.json
```
...
Resources:
  ...
  getAllNftsFunction:
    ...
    Properties:
      Handler: src/handlers/get-all-nfts.getAllNftsHandler
      Runtime: nodejs14.x
      DeadLetterQueue:
        Type: SQS
...
```bash
my-application$ sam logs -n putNftsFunction --stack-name nodejs --tail
```
...
```bash
aws cloudformation delete-stack --stack-name nodejs
```
